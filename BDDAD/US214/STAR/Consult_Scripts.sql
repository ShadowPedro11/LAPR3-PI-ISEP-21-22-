...
create or replace view lastFiveYearsEvolution as
    select T.Ano, Pr.Cultura as "ID Cultura", Pr.Tipo_Cultura as "Tipo Cultura",S.Nome as "Setor Nome", sum(P.Quantidade) as quantidade
    from Produção P
    ...
    group by T.Ano, Pr.Cultura, Pr.Tipo_Cultura, S.Nome
    order by T.Ano asc;
...
create or replace view evolucaoMensalPorTipoCultura as
    select T.Ano, T.Mes, Pr.Tipo_Cultura, sum(V.montante) as "MONTANTE"
    from Venda V inner join Tempo T on T.Tempo_id = V.Tempo_id
             inner join Produto Pr on Pr.Produto_id = V.Produto_id
    group by T.Ano, T.Mes, Pr.Tipo_Cultura
    order by Pr.Tipo_Cultura, T.Ano, T.Mes
