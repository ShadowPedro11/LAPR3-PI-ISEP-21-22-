...
public class MapGraph<V, E> extends CommonGraph<V, E> {
    ...
    public Collection<V> adjVertices(V vert) {
        if(vert == null)
            return null;
        for(Map.Entry<V, MapVertex<V, E>> mapVertexEntry : mapVertices.entrySet())
            if(mapVertexEntry.getKey().equals(vert))
                return mapVertexEntry.getValue().getAllAdjVerts();
        return null;
    }
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for(Map.Entry<V, MapVertex<V, E>> me : mapVertices.entrySet()){
            Edge<V, E> e = me.getValue().getEdge(vert);
            if(e != null)
                edges.add(e);
        }
    ...
    public String toString() {
        ...
        if (numVerts == 0) {
            ...
            s = numVerts + " entidades, " + numEdges + " ligações\n";
            for (MapVertex<V, E> mv : mapVertices.values())
                s += mv;
        }
        return s;
